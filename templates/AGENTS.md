...
- **CHECK_MODE (AI review scope):** Default `commit` — AI reviews only the last commit (HEAD~1..HEAD), stable. Alternatives: `snippet` (staged/unstaged or push range), `mix` (manual = full, push = commit), `full` (whole codebase chunked). Set in dashboard (Codex settings) or `.shimwrappercheckrc`.
...
- **Agent after failed AI review:** When the pipeline exits with AI review REJECT, it writes `.shimwrapper/review-failed.json` (verdict, score, review_file, timestamp) and prints a line starting with **`REVIEW_FAILED_AGENT_ACTION:`**. The agent should read that file and the referenced review in `.shimwrapper/reviews/`, then fix all deductions (broad pass per file), commit, and re-run the pipeline. After ACCEPT, the pipeline removes `review-failed.json`.
