...
# Usage: run-checks.sh [--frontend] [--backend] [--refactor|--until-95] [--no-frontend] [--no-backend] [--no-ai-review] [--no-explanation-check] [--no-i18n-check] ...
#   With no args: run frontend and backend checks. CHECK_MODE defaults to full (whole-codebase AI review).
#   --refactor / --until-95: force CHECK_MODE=full (chunked full scan). Pre-push sets CHECK_MODE=snippet (AI review only for pushed changes).
...
#   i18n check runs with frontend; use --no-i18n-check to disable (or SKIP_I18N_CHECK=1).
...
run_one() {
  ...
  case "$id" in
    ...
    gitleaks) if [[ "$run_gitleaks_rc" = "1" ]] && [[ "$run_gitleaks" = true ]]; then
            echo "Gitleaks..."; if command -v gitleaks >/dev/null 2>&1; then gitleaks detect --no-git --source . --verbose;
