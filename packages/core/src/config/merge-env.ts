...
export function mergeConfig(
  ...
  if (env.CHECK_MODE) {
    ...
    if (mode === "mix") mode = "full";
    if (mode === "diff") mode = "snippet";
  ...
  if (
    env.SHIM_AI_REVIEW_BLOCKING != null &&
  ...
  if (
    env.SHIM_STRICT_NETWORK_CHECKS != null &&
    env.SHIM_STRICT_NETWORK_CHECKS !== ""
  ) {
    merged.strictNetworkChecks = isRcEnabled(
      env.SHIM_STRICT_NETWORK_CHECKS,
      false,
    );
  }
  ...
  if (env.SHIM_AI_MIN_RATING) {
    ...
    if (Number.isFinite(n)) merged.aiReview.minRating = n;
  }
