...
export function parseRcValue(rawValue: string): string {
...
export function parseLegacyRcContent(content: string): Record<string, string> {
...
export function legacyRcToConfig(
  ...
  const draft = {
    ...
    gitCheckModeOnPush:
      rc.SHIM_GIT_CHECK_MODE_ON_PUSH === "snippet" ||
    ...
    i18nRequireMessagesDir: isRcEnabled(
      rc.SHIM_I18N_REQUIRE_MESSAGES_DIR,
      true,
    ),
    aiReview: {
...
function normalizeAiProvider(
  value: string | undefined,
): "auto" | "codex" | "api" | "custom" {
  const v = (value ?? "auto").toLowerCase();
  if (v === "codex" || v === "api" || v === "custom") return v;
...
export function serializeRcValue(value: string): string {
