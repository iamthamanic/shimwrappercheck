...
const FLAG_SKIP_MAP: Record<string, keyof RunChecksFlags> = {
  aiReview: "noAiReview",
...
export function parseRunChecksFlags(argv: string[]): RunChecksFlags {
  const has = (flag: string) => argv.includes(flag);
  let frontend = !has("--no-frontend");
  let backend = !has("--no-backend");
  if (has("--frontend")) frontend = true;
  if (has("--backend")) backend = true;
  if (
    !has("--frontend") &&
    !has("--backend") &&
    !has("--no-frontend") &&
...
export function buildRunChecksArgv(opts: {
  ...
  const argv: string[] = [];
  if (opts.frontend === false) argv.push("--no-frontend");
  if (opts.backend === false) argv.push("--no-backend");
  if (opts.noAiReview) argv.push("--no-ai-review");
  if (opts.noExplanationCheck) argv.push("--no-explanation-check");
  if (opts.noI18nCheck) argv.push("--no-i18n-check");
  if (opts.noSast) argv.push("--no-sast");
  if (opts.noGitleaks) argv.push("--no-gitleaks");
  if (opts.noFallow) argv.push("--no-fallow");
  if (opts.noRuff) argv.push("--no-ruff");
  if (opts.noShellcheck) argv.push("--no-shellcheck");
  if (opts.refactor) argv.push("--refactor");
  if (opts.until95) argv.push("--until-95");
  return argv;
}
...
export function resolveCheckOrder(
  ...
  const enabled = (id: string) => config.checks[id] !== false;

  return baseOrder.filter((id) => {
    if (!registeredIds.has(id)) return false;
    if (!enabled(id)) return false;
    if (FRONTEND_CHECK_IDS.has(id) && !flags.frontend) return false;
    if (BACKEND_CHECK_IDS.has(id) && !flags.backend) return false;
...
export function buildRunChecksEnv(
...
async function runAllChecksWithTimeout(
  options: RunAllChecksOptions,
): Promise<RunAllChecksResult> {
  const timeoutMs = options.timeoutSec! * 1000;
  let timer: ReturnType<typeof setTimeout> | undefined;

  const timeoutPromise = new Promise<RunAllChecksResult>((resolve) => {
    timer = setTimeout(() => {
      resolve(
        toRunAllChecksResult([
          {
            id: "runAllChecks",
            status: "infra_error",
            blocking: true,
...
async function runAllChecksInner(
  options: RunAllChecksOptions,
): Promise<RunAllChecksResult> {
