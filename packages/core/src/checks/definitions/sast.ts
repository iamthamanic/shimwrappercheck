...
export async function resolveSemgrepBaselineCommit(
  projectRoot: string,
  packageRoot: string,
...
export const sastCheck: CheckDefinition = {
  id: "sast",
  label: "Semgrep",
  category: "security",
  ...
  async run(ctx) {
    ...
    const baseline = await resolveSemgrepBaselineCommit(
      ctx.projectRoot,
      ctx.packageRoot,
      ctx.env,
    );
    ...
    if (result.exitCode === 127 || /not found/i.test(result.stderr)) {
      ...
      if (result.exitCode === 127) {
        return skip(
          "sast",
          "Semgrep not installed (pip install semgrep or npx semgrep)",
        );
      }
    }
