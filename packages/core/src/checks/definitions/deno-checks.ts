...
async function runDenoInBackend(
  ctx: Parameters<CheckDefinition["run"]>[0],
  id: string,
  args: string[],
  label: string,
): Promise<ReturnType<CheckDefinition["run"]>> {
  const backendDir = resolveBackendDir(
    ctx.projectRoot,
    ctx.config.backendPathPatterns,
  );
  if (!backendDir) {
...
export const denoFmtCheck: CheckDefinition = {
  id: "denoFmt",
  label: "Deno fmt",
  category: "backend",
  ...
  async run(ctx) {
    ...
    return runDenoInBackend(
      ctx,
      "denoFmt",
      ["fmt", "--check", backendDir],
      "Deno fmt",
    );
  },
};

export const denoLintCheck: CheckDefinition = {
  id: "denoLint",
  label: "Deno lint",
  category: "backend",
...
export const denoAuditCheck: CheckDefinition = {
  id: "denoAudit",
  label: "Deno audit",
  category: "backend",
