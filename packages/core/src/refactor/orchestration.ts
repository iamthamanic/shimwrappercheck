...
export function normalizeRefactorMode(raw: string | undefined): RefactorMode {
  const mode = String(raw ?? "off").toLowerCase();
  if (mode === "interactive" || mode === "agent") return mode;
  return "off";
}

/** Resolve refactor artifact paths (mirrors scripts/run-checks.sh). */
export function resolveRefactorPaths(
  projectRoot: string,
  ...
  return {
    refactorDir,
    todoFile: toAbs(
      env.SHIM_REFACTOR_TODO_FILE ??
        path.join(refactorDir, "refactor-todo.json"),
    ),
    stateFile: toAbs(
      env.SHIM_REFACTOR_STATE_FILE ??
        path.join(refactorDir, "refactor-state.json"),
    ),
    currentItemFile: toAbs(
      env.SHIM_REFACTOR_CURRENT_ITEM_FILE ??
...
export function resolveExtractRefactorScript(
  projectRoot: string,
  packageRoot: string,
): string | null {
  const candidates = [
...
export function findLatestReviewFile(reviewsDir: string): string | null {
...
export function buildRefactorHandoff(
...
): { message: string; openItems: RefactorTodoItem[]; state: RefactorState } {
  const todoDoc = readJson<{ items?: RefactorTodoItem[] }>(todoPath, {
    items: [],
  });
  const allItems = Array.isArray(todoDoc.items) ? todoDoc.items : [];
  const openItems = allItems.filter(
    (item) => item && item.status !== "done" && item.status !== "resolved",
  );
  const stateDoc = readJson<{ currentIndex?: number }>(statePath, {});
  ...
  if (options.advance) currentIndex += 1;
  ...
  fs.writeFileSync(
    currentItemPath,
    `${JSON.stringify(currentItemPayload, null, 2)}\n`,
    "utf8",
  );
 ...
 */
export async function runRefactorOrchestration(
  ...
  const extractScript = resolveExtractRefactorScript(
    options.projectRoot,
    options.packageRoot,
  );
  ...
  if (!extractScript) {
    console.error(
      "Refactor orchestration: extract-refactor-todo.sh not found; skipping TODO/current-item generation.",
    );
  ...
  if (!latestReview) {
    console.error(
      `Refactor orchestration: no review file found under ${paths.reviewsDir}; skipping.`,
    );
  ...
  const extract = await runCommand(
    "bash",
    [extractScript, latestReview, paths.todoFile],
    {
  ...
  if (!commandSucceeded(extract)) {
    console.error(
      `Refactor orchestration: failed to extract TODO from ${latestReview}.`,
    );
    return { skipped: true, reason: "extract failed", paths };
  }

  const handoff = buildRefactorHandoff(
    paths.todoFile,
    paths.stateFile,
    paths.currentItemFile,
    latestReview,
    mode,
    {
  ...
  if (mode === "interactive") {
    console.log(
      "Interactive resume: fix the current item, then run SHIM_REFACTOR_ADVANCE=1 ./scripts/run-checks.sh --refactor",
    );
  }
