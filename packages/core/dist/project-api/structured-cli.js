...
export function toggleLegacyCheck(envKey, enabled, projectRoot) {
    const result = setLegacyConfig({ [envKey]: enabled ? "1" : "0" }, projectRoot);
    return {
        ...result,
        envKey,
        enabled,
...
export function listChecksWithState(projectRoot) {
    const loaded = loadConfig({ projectRoot });
...
export function readLastErrorEntry(projectRoot) {
...
export function resolveReviewDirectory(projectRoot) {
    const { projectRoot: root } = getProjectPaths(projectRoot);
    ...
    if (fs.existsSync(presetsPath)) {
        try {
            ...
            if (typeof parsed.reviewOutputPath === "string" &&
...
export function findLatestReviewReport(projectRoot) {
...
export function getAgentsMdContent(projectRoot) {
    const { projectRoot: root } = getProjectPaths(projectRoot);
...
//# sourceMappingURL=structured-cli.js.map