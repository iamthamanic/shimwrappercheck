...
import { buildAiScriptEnv, isAiReviewBlocking, mapAiScriptResult, resolveCheckMode, runAiReviewScript, } from "../checks/definitions/ai-script-runner.js";
...
import { passesReview, parseReviewJsonFromText, redactReviewText, } from "./parse-review-output.js";
import { buildReviewPromptHead } from "./prompt.js";
import { canRunCoreAiReview, resolveAiProvider, resolveAiReviewEngine, resolveCodexBinary, } from "./provider-resolver.js";
import { runApiOrCustomReview } from "./api-runner.js";
function toInt(value, fallback) {
    const n = Number.parseInt(String(value ?? ""), 10);
    return Number.isFinite(n) && n > 0 ? n : fallback;
}
function resolveReviewsDir(ctx) {
...
function mapCoreReviewResult(passReview, blocking, details) {
    if (passReview) {
 ...
 */
export async function runAiReview(ctx) {
    ...
    const timeoutMs = (ctx.config.aiReview.timeoutSec ?? toInt(ctx.env.SHIM_AI_TIMEOUT_SEC, 180)) *
        1000;
    ...
    if (codexResult.exitCode !== 0) {
        return mapAiScriptResult("aiReview", "AI Review", {
            command: codex.path,
            args: ["exec"],
            cwd: ctx.projectRoot,
    ...
    if (!branch) {
        const branchResult = await defaultGitRunner(git.path, ["rev-parse", "--abbrev-ref", "HEAD"], ctx.projectRoot);
        branch = branchResult.stdout.trim() || "unknown";
    }
    fs.writeFileSync(reviewFile, formatReviewMarkdown({
        mode,
        branch,
        parsed,
        pass: passReview,
    ...
    const reviewFailedJson = path.join(path.dirname(reviewsDir), "review-failed.json");
    if (passReview) {
        try {
        ...
        fs.writeFileSync(reviewFailedJson, JSON.stringify({
...
//# sourceMappingURL=run-ai-review.js.map