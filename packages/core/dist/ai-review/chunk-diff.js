...
export const defaultGitRunner = async (gitPath, args, cwd) => {
...
export function limitDiffBytes(content, limitBytes) {
    const bytes = Buffer.byteLength(content, "utf8");
    if (bytes <= limitBytes * 2)
        return content;
 ...
 */
export async function buildCommitDiff(projectRoot, gitPath, runGit = defaultGitRunner) {
    const hasParent = await runGit(gitPath, ["rev-parse", "--verify", "HEAD~1"], projectRoot);
    const range = hasParent.exitCode === 0 ? "HEAD~1..HEAD" : `${EMPTY_TREE}..HEAD`;
    const diff = await runGit(gitPath, ["diff", "--no-color", range], projectRoot);
 ...
 */
export async function buildSnippetDiff(projectRoot, gitPath, runGit = defaultGitRunner) {
    let content = "";
    ...
    const staged = await runGit(gitPath, ["diff", "--cached", "--no-color"], projectRoot);
    ...
    if (!content.trim()) {
        const upstream = await runGit(gitPath, ["rev-parse", "--abbrev-ref", "--symbolic-full-name", "@{u}"], projectRoot);
        let range = "";
        if (upstream.exitCode === 0 && upstream.stdout.trim()) {
            range = "@{u}...HEAD";
        }
        else {
            const parent = await runGit(gitPath, ["rev-parse", "--verify", "HEAD~1"], projectRoot);
            if (parent.exitCode === 0)
                range = "HEAD~1...HEAD";
        }
        if (range) {
            const ranged = await runGit(gitPath, ["diff", "--no-color", range], projectRoot);
...
export async function buildDiffForMode(mode, projectRoot, gitPath, runGit) {
    if (mode === "commit") {
        return buildCommitDiff(projectRoot, gitPath, runGit);
    }
    return buildSnippetDiff(projectRoot, gitPath, runGit);
}
//# sourceMappingURL=chunk-diff.js.map