...
export function readRcHeaderLine(rcPath) {
    ...
    return (header ?? "# shimwrappercheck config (managed by shimwrappercheck CLI)");
}
...
export function patchLegacyRc(values, options = {}) {
    const loaded = loadConfig(options);
    const mergedLegacy = { ...loaded.legacy, ...values };
    const next = mergeConfig(legacyRcToConfig(mergedLegacy, { projectRoot: loaded.config.projectRoot }), options.projectRoot
