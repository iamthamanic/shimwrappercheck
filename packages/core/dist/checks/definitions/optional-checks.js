...
import { skip, runExternalTool, projectFileExists, fromCommandFailure, resolveToolCommand, } from "./helpers.js";
import { runCommand } from "../../runners/command-runner.js";
import fs from "node:fs";
export const licenseCheckerCheck = {
    id: "licenseChecker",
    ...
    async run(ctx) {
        ...
        return fromCommandFailure(ctx, "licenseChecker", result, "License checker", true);
    },
};
export const architectureCheck = {
    id: "architecture",
    ...
    async run(ctx) {
        if (!projectFileExists(ctx, ".dependency-cruiser.json")) {
            return skip("architecture", "Architecture: .dependency-cruiser.json not found");
        }
        const entry = projectFileExists(ctx, "src")
            ...
            : projectFileExists(ctx, "dashboard")
                ? "dashboard"
                : ".";
        return runExternalTool(ctx, "architecture", "npx", ["depcruise", entry, "--output-type", "err"], "Architecture (dependency-cruiser)", true);
    },
};
export const complexityCheck = {
    ...
    async run(ctx) {
        ...
        const templateConfig = path.join(ctx.packageRoot, "templates/eslint.complexity.json");
        let configPath = localConfig;
        if (!projectFileExists(ctx, "eslint.complexity.json")) {
            if (!fs.existsSync(templateConfig)) {
                return skip("complexity", "Complexity: eslint.complexity.json not found");
            }
            configPath = templateConfig;
        }
        const result = await runCommand(resolveToolCommand(ctx, "eslint"), [".", "-c", configPath], {
            cwd: ctx.projectRoot,
...
export const mutationCheck = {
    ...
    async run(ctx) {
        ...
        return runExternalTool(ctx, "mutation", "npx", ["stryker", "run"], "Stryker mutation", true);
    },
};
export const e2eCheck = {
    id: "e2e",
    label: "E2E",
    category: "other",
    envKey: "SHIM_RUN_E2E",
    ...
    async run(ctx) {
        const hasPlaywright = projectFileExists(ctx, "playwright.config.ts") ||
            projectFileExists(ctx, "playwright.config.js");
        if (!hasPlaywright) {
            return skip("e2e", "E2E: no Playwright config found");
        }
        return runExternalTool(ctx, "e2e", "npx", ["playwright", "test"], "E2E (Playwright)", true);
    },
};
