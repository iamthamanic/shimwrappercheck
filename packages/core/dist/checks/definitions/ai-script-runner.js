...
export function resolveCheckMode(ctx) {
    if (ctx.flags.refactor)
        return "full";
...
export function isAiReviewBlocking(ctx) {
    return (ctx.config.aiReview.blocking ||
...
export function isExplanationBlocking(ctx) {
    return (ctx.env.SHIM_EXPLANATION_BLOCKING === "1" ||
        String(ctx.env.SHIM_EXPLANATION_BLOCKING).toLowerCase() === "true");
}
/** Resolve AI provider (auto prefers codex when script exists). */
export function resolveAiProvider(ctx) {
    const configured = ctx.config.aiReview.provider ?? "auto";
    if (configured !== "auto")
        return configured;
    if (scriptExists(ctx, "scripts/ai-code-review.sh"))
        return "codex";
    if (scriptExists(ctx, "scripts/ai-deductive-review.js"))
        return "api";
    if (scriptExists(ctx, "scripts/ai-code-review.js"))
        return "custom";
    return "codex";
}
function scriptExists(ctx, relativePath) {
...
export function resolveScriptPath(ctx, relativePath) {
    const candidates = [
        path.join(ctx.projectRoot, relativePath),
        path.join(ctx.packageRoot, relativePath),
        path.join(ctx.projectRoot, "node_modules", "shimwrappercheck", relativePath),
    ];
...
export function buildAiScriptEnv(ctx, extra = {}) {
...
export function mapAiScriptResult(id, label, result, blocking) {
...
export async function runAiReviewScript(ctx) {
    ...
    if (provider === "api") {
        ...
        if (!script) {
            return skip("aiReview", "AI review (api): ai-deductive-review.js not found");
        }
    ...
    if (provider === "custom") {
        ...
        if (!script) {
            return skip("aiReview", "AI review (custom): ai-code-review.js not found");
        }
        const result = await runCommand(process.execPath, [script], {
            cwd: ctx.projectRoot,
            env,
...
export async function runExplanationScript(ctx) {
    ...
    if (provider === "custom") {
        ...
        if (!script) {
            return skip("explanationCheck", "Full Explanation (custom): ai-explanation-check.js not found");
        }
        const result = await runCommand(process.execPath, [script], {
            cwd: ctx.projectRoot,
            env,
        ...
        return mapAiScriptResult("explanationCheck", "Full Explanation", result, blocking);
    }
    const script = resolveScriptPath(ctx, "scripts/ai-explanation-check.sh");
    if (!script) {
        return skip("explanationCheck", "Full Explanation: ai-explanation-check.sh not found");
    }
    const result = await runCommand("bash", [script], {
        cwd: ctx.projectRoot,
        env,
...
//# sourceMappingURL=ai-script-runner.js.map