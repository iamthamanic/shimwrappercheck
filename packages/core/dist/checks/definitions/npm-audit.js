...
export const npmAuditCheck = {
    ...
    requiredTools: ["npm"],
    async run(ctx) {
        const level = ctx.config.auditLevel ?? ctx.env.SHIM_AUDIT_LEVEL ?? "moderate";
        const result = await runCommand("npm", ["audit", "--audit-level", String(level)], { cwd: ctx.projectRoot, env: ctx.env, timeoutMs: 300_000 });
