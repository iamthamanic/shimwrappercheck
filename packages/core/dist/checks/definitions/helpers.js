...
export function pass(id, message, blocking = true) {
    return { id, status: "passed", blocking, message };
}
/** Build a skipped check result. */
export function skip(id, message) {
...
export function fromCommandFailure(ctx, id, result, label, optionalCheck = false) {
...
export async function runNpmScriptIfPresent(ctx, id, scriptName, label) {
...
export async function runNodeScriptIfExists(ctx, id, relativePaths, label) {
...
export async function runBashScriptIfExists(ctx, id, relativePath, label) {
...
export async function runNpxTool(ctx, id, tool, args, label, optionalCheck = false) {
...
export function resolveToolCommand(ctx, toolName) {
...
export async function runExternalTool(ctx, id, command, args, label, optionalCheck = false, preferChecktools = false, envOverride) {
...
export function projectFileExists(ctx, name) {
