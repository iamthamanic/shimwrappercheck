...
const EXCLUDE = "node_modules,.git,.next,dist,build,.shim,.shimwrapper,.stryker-tmp,.codex-home";
...
export const ruffCheck = {
    ...
    async run(ctx) {
        ...
        result = await runCommand("ruff", ["format", "--check", ".", "--force-exclude", "--exclude", EXCLUDE], { cwd: ctx.projectRoot, env: ctx.env, timeoutMs: 300_000 });
