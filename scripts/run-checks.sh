...
run_ai_review_custom() {
	if [[ -f "$ROOT_DIR/scripts/ai-code-review.js" ]]; then
		node "$ROOT_DIR/scripts/ai-code-review.js"
	elif [[ -f "$PROJECT_ROOT/node_modules/shimwrappercheck/scripts/ai-code-review.js" ]]; then
		node "$PROJECT_ROOT/node_modules/shimwrappercheck/scripts/ai-code-review.js"
	else
		echo "Skipping custom AI review: ai-code-review.js not found." >&2
	fi
}
...
if [[ $# -eq 0 ]]; then
	...
	for arg in "$@"; do
		...
		*)
			echo "Unknown option: $arg. Use --frontend, --backend, --refactor, --until-95, --no-frontend, --no-backend, --no-ai-review, --no-explanation-check, --no-i18n-check, --no-sast, --no-gitleaks, --no-license-checker, --no-architecture, --no-complexity, --no-mutation, --no-e2e, --no-ruff, --no-shellcheck." >&2
			exit 1
			;;
		esac
	done
...
run_one() {
	...
	aiReview)
		if [[ "$run_ai_review_rc" = "1" ]] && [[ "$run_ai_review" = true ]]; then
			...
			if [[ "$ai_provider" == "api" ]]; then
				...
				echo "AI Review provider: custom"
				run_ai_review_custom
				rc=$?
			else
	...
	explanationCheck)
		if [[ "$run_explanation_check_rc" = "1" ]] && [[ "$run_explanation_check" = true ]]; then
			local expl_provider
			expl_provider="$(resolve_ai_review_provider)"
			if [[ "$expl_provider" == "custom" ]]; then
				echo "Full Explanation check..."
				node "$ROOT_DIR/scripts/ai-explanation-check.js"
				rc=$?
			else
				echo "Full Explanation check..."
