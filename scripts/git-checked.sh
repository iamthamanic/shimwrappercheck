...
if is_push_command && [[ -n "$GIT_CMD" ]] && [[ -x "$GIT_CMD" ]]; then
	if "$GIT_CMD" rev-parse --abbrev-ref --symbolic-full-name "@{u}" >/dev/null 2>&1; then # Upstream vorhanden.
		...
		if [[ -n "$AHEAD" ]] && [[ "$AHEAD" =~ ^[0-9]+$ ]] && [[ "$AHEAD" -gt 1 ]]; then
			upstream_symref="@{u}"
			echo "Pre-push: Multiple local commits ($AHEAD) ahead of upstream. AI review only reviews the latest commit. Squash (e.g. git rebase -i $upstream_symref) or push one commit at a time." >&2
