...
export interface CheckSettings {
  ...
  aiReview?: {
    ...
    minRating?: number;
    reviewDir?: string;
    checkMode?: "mix" | "snippet" | "diff" | "full" | "commit";
...
export interface SettingsData {
  ...
  /** When false, shim runs no checks and no hooks (passthrough only). UI: "Checks" vs "No checks". */
  shimEnabled?: boolean;
}
...
/** Default: no checks in My Shim (all in Check Library). Shim runs checks when shimEnabled is true. */
...
export function buildRcContent(settings: SettingsData): string {
  ...
  lines.push(`SHIM_ENABLED=${shimEnabled ? 1 : 0}`);
