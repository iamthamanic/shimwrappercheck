...
- **CHECK_MODE (AI review scope):** Default \`snippet\` — AI reviews only changed code snippets. Whole codebase: \`CHECK_MODE=full npm run checks\` or \`CHECK_MODE=full bash scripts/run-checks.sh\`. Mix loop: \`CHECK_MODE=mix\` (manual full scan, pre-push snippet). Other checks always run on the full codebase; only the AI review input switches. Set in dashboard (Codex settings) or \`.shimwrappercheckrc\`.
