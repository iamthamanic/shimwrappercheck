...
const defaultAgentsContent = `# Agent instructions
...
- **AI review (provider-based):** Runs by default after frontend/backend checks. Provider via \`SHIM_AI_REVIEW_PROVIDER=auto|codex|api\` (auto prefers Codex, fallback API-key review). Skip with \`--no-ai-review\` or \`SKIP_AI_REVIEW=1\` for the shim; **on git push** many projects require AI review (no skip). **Pass criteria:** **score ≥ 95%** and **verdict ACCEPT** (strict Senior-Architect checklist: SOLID, DRY, performance, security, robustness, maintainability; deductions per violation). If the review fails, fix the code and re-run until it passes. Reviews are saved to \`.shimwrapper/reviews/\` (gitignored). When you run the shim or push and the output includes the AI review, include token usage and the review result (PASS/FAIL, score, deductions, verdict) in your response.
- **CHECK_MODE (AI review scope):** Default \`commit\` — AI reviews only the last commit (HEAD~1..HEAD), stable. Alternatives: \`snippet\` (staged/unstaged or push range), \`mix\` (manual = full, push = commit), \`full\` (whole codebase). Set in dashboard (Codex settings) or \`.shimwrappercheckrc\`.
- **When AI review REJECTs — address broadly:** Do not fix only the single mentioned point. For each affected file from the deductions, do a full pass over the checklist: SOLID/DRY, IDOR/auth, rate limiting, input validation, error handling, edge cases. Then commit and re-run the review. See \`docs/AI_REVIEW_WHY_NEW_ERRORS_AFTER_FIXES.md\` for rationale and route checklist.
- **Agent after failed AI review:** On REJECT the pipeline writes \`.shimwrapper/review-failed.json\` and prints \`REVIEW_FAILED_AGENT_ACTION:\`. Read that file and the referenced review, fix all deductions (broad pass per file), commit, then re-run the pipeline. After ACCEPT, \`review-failed.json\` is removed.
