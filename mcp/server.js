...
function writeRcFile(filePath, values) {
  ...
  const existingLines = fs.existsSync(filePath)
    ...
    : [];
  const headerLine = existingLines.find((l) => l.trim().startsWith("#")) || "# shimwrappercheck config (managed by MCP)";
  ...
  for (const catalogPath of candidates) {
    if (fs.existsSync(catalogPath)) {
      try {
        ...
        continue; // Nächsten Pfad versuchen; ohne würden wir aufgeben.
      }
    }
  }
...
function writeTomlMcpConfig(configPath, config) {
  ...
  const newEntry = generateTomlMcpEntry("shimwrappercheck", config.mcpServers?.shimwrappercheck || {});
...
function configureMcpClient(client, serverPath) {
  ...
  const resolvedServerPath =
    serverPath ||
    path.resolve(__dirname, "server.js"); // Eigene server.js-Position; ohne wüsste der Agent nicht, wo der Server liegt.
  ...
  switch (toolName) {
    ...
    case "get_check_status": {
      ...
      return lastError
        ...
        : { hasError: false, message: "No last error found. Last run passed or no run yet." };
    }
      ...
      if (catalog.length === 0) {
        ...
        const fallbackChecks = Object.keys(config)
          ...
          .map((k) => {
            ...
            return { id, envKey: k, enabled: config[k] !== "0", defaultEnabled: true, label: id };
          });
    ...
    case "get_agents_md": {
      ...
      try {
        ...
        return { found: true, path: agentsPath, content: content.slice(0, 50000) }; // Auf 50 KB begrenzen; ohne droht Overflow bei großen Dateien.
      } catch {
...
function main() {
  ...
  console.error("[MCP shimwrappercheck] Server started on stdio. Project root:", projectRoot);
}
