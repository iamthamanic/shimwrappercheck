...
- **Checks until clean:** Run checks until there are no errors and no warnings; fix and re-run until all pass. Do not push with failing or warning checks. Pushing without running checks (e.g. `git push --no-verify`) is not allowed; quality is enforced by the pre-push hook and the shim.
...
- **CHECK_MODE (AI review scope):** When you run `scripts/run-checks.sh` (or `npm run checks`) from the CLI, CHECK_MODE defaults to **full** (chunked whole-codebase scan). Use `--refactor` or `--until-95` to force full. **Pre-push always runs AI review in CHECK_MODE=commit** (only the last commit, HEAD~1..HEAD). It cannot be disabled on push: `--no-ai-review` and `SKIP_AI_REVIEW` are ignored; push is blocked until AI review passes (ACCEPT, score ≥ 95). With `CHECK_MODE=full` the review runs **chunked per directory**: only existing dirs among `src`, `supabase`, `scripts`, `dashboard` are used; for each dir a separate diff (`git diff EMPTY_TREE..HEAD -- <dir>`) is sent to Codex, up to 150 KB per chunk. One Codex run per chunk (timeout 600 s per chunk, configurable via `SHIM_AI_CHUNK_TIMEOUT`). PASS only if all chunks get ACCEPT and score ≥ 95. With `CHECK_MODE=snippet` only the staged/unstaged or to-be-pushed changes are reviewed (one run). Review files are named `review-snippet-…`, `review-commit-…`, or `review-full-…`. Can be set in the dashboard (Codex settings) or in `.shimwrappercheckrc`.
- **Mix workflow (Full-Scan → fix → Push with Commit-Review → repeat):** (1) **Full-Scan:** Run `./scripts/run-checks.sh` or `./scripts/run-checks.sh --refactor` (or `--until-95`). All chunks (src, supabase, scripts, dashboard) run; all issues visible. Loop: fix → commit → run again until all chunks ≥ 95%. (2) **Push:** Work through deductions, commit, then push. Pre-push runs with **CHECK_MODE=commit** (AI review only for the last commit; must pass or push is blocked). (3) **Repeat:** Run Full-Scan again; if not yet ≥ 95%, fix → commit → push (again commit-mode review) → Full-Scan. See `docs/REFACTOR_MIX_WORKFLOW.md`.
- **When AI review REJECTs — address broadly:** Do not fix only the single mentioned point. For each affected file from the deductions, do a full pass over the checklist: SOLID/DRY, IDOR/auth, rate limiting, input validation, error handling, edge cases. Then commit and re-run the review. See `docs/AI_REVIEW_WHY_NEW_ERRORS_AFTER_FIXES.md` for rationale and route checklist. The AI review itself uses a fixed checklist (SOLID, DRY, performance, security, robustness, maintainability); deductions apply per violation.
