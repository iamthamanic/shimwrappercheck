...
- **CHECK_MODE (AI review scope):** When you run `scripts/run-checks.sh` (or `npm run checks`) from the CLI, CHECK_MODE defaults to **full** (chunked whole-codebase scan). Use `--refactor` or `--until-95` to force full. **Pre-push sets CHECK_MODE=snippet** so the AI review runs only on the pushed changes (diff); run-checks.sh does not override CHECK_MODE if it is already set. With `CHECK_MODE=full` the review runs **chunked per directory**: only existing dirs among `src`, `supabase`, `scripts`, `dashboard` are used; for each dir a separate diff (`git diff EMPTY_TREE..HEAD -- <dir>`) is sent to Codex, up to 150 KB per chunk. One Codex run per chunk (timeout 600 s per chunk, configurable via `SHIM_AI_CHUNK_TIMEOUT`). PASS only if all chunks get ACCEPT and score ≥ 95. With `CHECK_MODE=snippet` only the staged/unstaged or to-be-pushed changes are reviewed (one run). Review files are named `review-snippet-…` or `review-full-…`. Can be set in the dashboard (Codex settings) or in `.shimwrappercheckrc`.
- **Mix workflow (Full-Scan → fix → Push with Snippet-Review → repeat):** (1) **Full-Scan:** Run `./scripts/run-checks.sh` or `./scripts/run-checks.sh --refactor` (or `--until-95`). All chunks (src, supabase, scripts, dashboard) run; all issues visible. Loop: fix → commit → run again until all chunks ≥ 95%. (2) **Push:** Work through deductions, commit, then push. Pre-push runs with **CHECK_MODE=snippet** (schnelle Checks + AI-Review nur fürs Diff). (3) **Repeat:** Run Full-Scan again; if not yet ≥ 95%, fix → commit → push (again snippet-only review) → Full-Scan. See `docs/REFACTOR_MIX_WORKFLOW.md`.
